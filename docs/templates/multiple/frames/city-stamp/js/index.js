...
import { getStampURLFn } from './stamp.js'
...
window.joppInit = async (opts = demoData) => {
  ...
  let tocPageNum = 0;
  const stampURL = await getStampURLFn(templateInfo.options?.stampURL)
    ...
    switch (page.type) {
      ...
      case "content": {
        ...
        if (contentPageNum == totalContentPage) {
          ...
          if (stampURL) {
            stampEl.setAttribute('src', stampURL)
          } else {
            stampEl.setAttribute('src', `./imgs/stamp_${getRandom(1,7)}.png`)
          }
